export FZF_DEFAULT_OPTS="
  ...
  --bind 'ctrl-e:execute(echo {} | xclip -selection clipboard)+abort'
  --bind 'ctrl-w:execute(echo {} | xclip -selection clipboard)'
  --bind 'ctrl-alt-w:execute(echo {+} | xargs -d \" \" -I{} printf \"%s\\n\" {} | tac | xclip -selection clipboard)'
  --color='preview-bg:#222222,hl:#1e90ff,hl+:#1e90ff,bg+:#444444,border:#222222,pointer:#1e90ff,scrollbar:#1e90ff,preview-scrollbar:#1e90ff'
  --pointer=' '
  --marker='●∙'
  --scrollbar='▉'
"
...
unique_history() {
  eval "$(history | awk '{ key=""; for (i=4; i<=NF; i++) key = key $i OFS; if (!seen[key]++) print }' | sort -n -k1,1 | fzf --tac | awk '{for (i=4; i<=NF; i++) printf "%s ", $i; print ""}')"
}
