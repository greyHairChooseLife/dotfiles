...
local function open_oil_relatively(mode) -- oil 켜기
    ...
    if node then
        ...
        if path and (path == "/" or path == vim.fn.fnamemodify(path, ":h")) then path = node.absolute_path end
...
-- keymap
map({ "n", "v" }, "-", function() open_oil_relatively() end, opts)
map({ "n", "v" }, "_", function() open_oil_relatively("tab") end, opts)
map({ "n", "v" }, "i", function()
