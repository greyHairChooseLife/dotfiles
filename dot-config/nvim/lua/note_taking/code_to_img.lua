...
local convert = function(range, opts)
    ...
    for i, line in pairs(html) do
        if line:match("^%s*body") then html[i] = bodyStyle .. containerStyle .. preStyle end

        if line:match("^%s*<pre>") then html[i] = "<div class='container'><pre>" end

        if line:match("^%s*</pre>") then html[i] = "</pre></div>" end
    end

    local wk_args = { "wkhtmltoimage", "-" }
    if format == "pdf" then wk_args = { "wkhtmltopdf", "-" } end
...
local visual_convert = function(opts)
    ...
    if mode == "n" then
        ...
        convert({ startLine, endLine }, { save_to_file = save_to_file, format = format, save_path = save_path, filename = filename })
    end
end
