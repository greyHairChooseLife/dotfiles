local restore_messages = require("AI.codecompanion.utils.deprecated.restore_messages")
...
local function load_session(file_path)
    local success, result = pcall(function()
        ...
        local data = chunk()
        if not data or not data.messages then error("Invalid session file format") end
        return data
    end)
            ...
            finder = finders.new_table({
                ...
                entry_maker = function(file)
                    ...
                    for i, msg in ipairs(result.messages) do
                        ...
                        if #msg.content > 100 then preview = preview .. "..." end
                        local role_display = msg.role
                        if msg.opts and msg.opts.visible == false then role_display = role_display .. " [hidden]" end
