...
local function generate_commit_message()
    ...
    if handle_staged == nil and handle_staged == nil and handle_untracked == nil then return nil end
...
Full diffs are as follows:
    ...
    if #unstaged > 0 then
        content = content .. "== Unstaged Changes Start(`git diff`) ==\n~~~diff\n" .. unstaged .. "\n~~~\n== Unstaged Changes End(`git diff`) ==\n\n"
    end
    if #untracked > 0 then
        content = content .. "== Untracked Files(`git ls-files --others --exclude-standard`) ==\n~~~plaintext\n" .. untracked .. "\n~~~\n\n"
