...
local add_reference = require("AI.codecompanion.utils.deprecated.add_reference")
...
local function read_lines(action)
    ...
    for line in lines do
        ...
        if current_line >= start_line and current_line <= end_line then table.insert(extracted, current_line .. ":  " .. line) end
...
local function search_replace(action)
    ...
    if not content then error("No data found in " .. action.path) end

    local changed, substitutions_count = content:gsub(vim.pesc(action.search), vim.pesc(action.replace))
    if substitutions_count == 0 then error("Could not find the search string in " .. action.path) end
...
return {
    ...
    cmds = {
        ...
        function(self, action, input)
            ...
            if not ok then return { status = "error", msg = data, data = data } end
...
HINT: If search/replace doesn't work, refetch and retry. If it still doesn't work, stop and ask for help.
    ...
    handlers = {
        ...
        ---@return nil
        on_exit = function(agent) file = nil end,
    },
    output = {
