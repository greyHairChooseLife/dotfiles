...
local deltas = {}
local function add_delta(bufnr, line, delta) table.insert(deltas, { bufnr = bufnr, line = line, delta = delta }) end
...
local function intersect(bufnr, line)
    ...
    for _, v in ipairs(deltas) do
        if bufnr == v.bufnr and line > v.line then delta = delta + v.delta end
    end
    return delta
end
...
local function delete(bufnr, action)
    ...
    if action.all then
        ...
        if end_line == 0 then end_line = 1 end
    end
...
local function add(bufnr, action)
    if not action.line and not action.replace then assert(false, "No line number or replace request provided by the LLM") end
    ...
    cmds = {
        ...
        function(self, actions, input)
            ...
            local function run(action)
                ...
                if not action.buffer then return { status = "error", msg = "No buffer number provided by the LLM" } end
            ...
            if vim.isarray(actions) then
                for _, v in ipairs(actions) do
                    output = run(v)
                    if output.status == "error" then break end
                end
            else
                output = run(actions)
            end
...
IMPORTANT: Buffer number must be valid. You should either fetch it from user or via other tools.
    ...
    output = {
        ...
        error = function(agent, cmd, stderr, stdout)
            if type(stderr) == "table" then stderr = vim.iter(stderr):map(function(v) return v end):join("\n") end
            return agent.chat:add_buf_message({
                role = config.constants.USER_ROLE,
                content = string.format(
