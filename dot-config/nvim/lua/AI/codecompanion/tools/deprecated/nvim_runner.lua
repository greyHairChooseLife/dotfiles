...
local function is_high_risk(action)
    ...
    if type == "lua_exec" then
        ...
        if code:match("nvim_buf_delete") or code:match("nvim_command") or code:match("vim%.fn%.delete") or code:match("os%.execute") or code:match("io%.") then
            return true
        end
        -- Check code length
        if #code > 150 then return true end
    end
...
return {
    ...
    cmds = {
        ...
        function(_, action, _)
            ...
            if not actions[action_type] then return { status = "error", msg = "Unknown action type: " .. action_type } end

            local ok, result = pcall(actions[action_type], action)
            if not ok then return { status = "error", msg = result, data = result } end
  ...
  - element `code`
    ...
    output = {
        ...
        prompt = function(agent, self)
            ...
            if action_list._attr ~= nil then action_list = { action_list } end
            -- Create prompts for different operation types
            local prompts = {
                vim_cmd = function(a) return string.format("vim: `%s`", a.command) end,

                lua_exec = function(a)
                    -- Truncate long code display
                    local code = a.code
                    if #code > 60 then code = code:sub(1, 57) .. "..." end
            ...
            for _, action in ipairs(action_list) do
                local prompt_fn = prompts[action._attr.type] or function(_) return string.format("Execute %s operation?", string.upper(action._attr.type)) end
                local prompt = prompt_fn(action)
                -- Add warning for high-risk operations
                if is_high_risk(action) then prompt = "⚠️ HIGH RISK OPERATION! " .. prompt end
                res = res .. prompt .. "\n"
            end
            return res
        end,
        ...
        rejected = function(agent)
            if not vim.g.codecompanion_auto_tool_mode then agent.status = "rejected" end
