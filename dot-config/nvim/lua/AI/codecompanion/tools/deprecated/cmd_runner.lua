...
IMPORTANT: You cannot interact with terminal or running processes, so all your commands should be non-interactive, do not try commands like `npm run dev`, `npm run start`, or start a server, or something else. If you really need to do that, you should ask user for do so.
    ...
    handlers = {
        ...
        setup = function(agent)
            ...
            for _, act in ipairs(actions) do
                ...
                if act.flag then entry.flag = act.flag end
    ...
    output = {
        ...
        prompt = function(agent, self)
            ...
            if vim.tbl_count(cmds) == 1 then return string.format("Run the command `%s`?", table.concat(cmds[1].cmd, " ")) end

            local individual_cmds = vim.tbl_map(function(c) return table.concat(c.cmd, " ") end, cmds)
        ...
        error = function(agent, cmd, stderr, stdout)
            ...
            if stdout and not vim.tbl_isempty(stdout) then to_chat("Also some stdout from", agent, { cmd = cmd.cmd or cmd, output = stdout }) end
        end,
        ...
        success = function(agent, cmd, stdout)
            ...
            if agent.stderr then
                ...
                if type(agent.stderr) == "table" then stderr = vim.iter(agent.stderr):flatten():join("\n") end
                if stderr ~= "" then to_chat("Also some stderr from", agent, { cmd = cmd.cmd or cmd, output = stderr }) end
            end
        end,
    },
}
