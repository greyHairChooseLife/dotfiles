...
local function add_buf_message(chat, data, opts)
    ...
    if data.role == config.constants.SYSTEM_ROLE then return end
    ...
    local function update_buffer()
        ...
        if new_response then chat.ui:render_headers() end

        if chat.last_role ~= config.constants.USER_ROLE then chat.ui:lock_buf() end
    ...
    -- Handle a new role
    if (data.role and data.role ~= chat.last_role) or (opts and opts.force_role) then new_role() end
