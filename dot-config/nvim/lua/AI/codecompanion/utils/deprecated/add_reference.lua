...
local function ts_parse_buffer(chat)
    ...
    for id, node in query:iter_captures(root, chat.bufnr, chat.header_line - 1, -1) do
        if query.captures[id] == "refs" then refs = node end
    end
...
local function references_add(self, ref)
    if not ref or not config.display.chat.show_references then return self end

    local existed = false
    if ref then
        if not ref.opts then ref.opts = {} end
        ...
        if not existed then table.insert(self.Chat.refs, ref) end
        -- If it's a buffer reference and it's being watched, start watching
        if ref.bufnr and ref.opts.watched then self.Chat.watchers:watch(ref.bufnr) end
    end
