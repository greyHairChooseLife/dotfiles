...
function ReloadLayout(force)
    ...
    if force then require("UI.modules.window_size").unfixAllWindows() end
    ...
    -- check nvim-tree
    if is_tree_open then NvimTreeResetUI() end
...
function RunBufferWithSh(opt)
    ...
    if selected and underline then
        ...
        if cmd and #cmd > 0 and cmd:sub(-1) == "\n" then cmd = cmd:sub(1, -2) end
        vim.api.nvim_command("silent! read !bash -ic " .. vim.fn.shellescape(cmd .. " | sed -r 's/\\x1B\\[[0-9;]*[mK]//g'") .. " 2>/dev/null") -- interactive shell without Color
        return
    end
    ...
    vim.api.nvim_command([[r !date "+\%T" | awk '{line="=========================="; print line "\n===== time: " $1 " =====\n" line}']])
    ...
    for _, line in ipairs(lines) do
        if line:match("%S") then -- Skip empty lines
            ...
            vim.api.nvim_command("silent! read !bash -ic " .. vim.fn.shellescape(line .. " | sed -r 's/\\x1B\\[[0-9;]*[mK]//g'") .. " 2>/dev/null") -- Add the output
    ...
    if current_height < line_count + 5 then vim.api.nvim_win_set_height(0, line_count + 5) end
...
function Safe_search(direction)
    ...
    local success, _ = pcall(function() vim.cmd("normal! " .. direction) end) -- 검색 실행
