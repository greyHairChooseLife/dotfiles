...
-- Diagnostics
map("n", ",.d", function()
	snp.diagnostics_buffer({ layout = "ivy_split" })
end)
map("n", ",.D", function()
	snp.diagnostics({ layout = "ivy_split" })
end)
map("n", ",.s", function()
	snp.lsp_symbols({ layout = "lsp_select" })
end)

-- Etc
map("n", ",.,.", function()
	snp.pick({ layout = "select" })
end)
map("n", ",.C", my_picker_src.command_history)
map("n", ",.r", function()
	snp.registers({ layout = "select" })
end)
map("n", ",.q", function()
	snp.qflist({ layout = "right" })
end)
map("n", ",.m", function()
	snp.marks({ layout = "dropdown" })
end)
map("n", ",.R", snp.resume)
map("n", ",.H", function()
	snp.help({ layout = "ivy_split" })
end)
map("n", ",.N", function()
	snp.pick({ source = "noice", layout = "ivy_split" })
end)
