return {
    ...
    config = function()
        ...
        local focus_preview = function(prompt_bufnr)
            ...
            vim.keymap.set({ "n", "v" }, "gq", function() actions.close(prompt_bufnr) end, { buffer = bufnr })
            vim.keymap.set("n", "i", function() vim.cmd(string.format("noautocmd lua vim.api.nvim_set_current_win(%s)", prompt_win)) end, { buffer = bufnr })
        ...
        local function focus_or_open(prompt_bufnr)
            ...
            local function is_file_in_buffer_list(filepath)
                ...
                for _, buf in ipairs(buffers) do
                    if vim.api.nvim_buf_is_loaded(buf) and (vim.api.nvim_buf_get_name(buf) == filepath or buf == bufnr) then return true, buf end
                end
                return false
            end
            ...
            if is_loaded then
                ...
                if not found_win and buf ~= nil then vim.api.nvim_set_current_buf(buf) end
            -- For already loaded buffers, don't change cursor position
            else
                actions.file_edit(prompt_bufnr)

                -- Only set cursor position for newly opened files
                vim.defer_fn(function() vim.api.nvim_win_set_cursor(0, { lnum, lcol - 1 }) end, 20)
            end
        ...
        local select_one_or_multi = function(prompt_bufnr, variant)
            ...
            if variant ~= "T" and require("utils").tree:is_visible() then NvimTreeResetUI() end
        end
