...
function ManageBuffer_gq(bufnr, winid)
    ...
    if not vim.api.nvim_buf_is_valid(bufnr) then return vim.notify(bufnr .. " is not valid bufrn.", 3, { render = "minimal" }) end
...
function ManageBuffer_gtq()
    ...
    if is_special_tabs then return vim.cmd("tabclose!") end
...
function ManageBuffer_gtQ() vim.notify("그간의 경험을 돌아보면 딱히 쓴 일, 쓸 일이 없는데?", 1, { render = "minimal" }) end
