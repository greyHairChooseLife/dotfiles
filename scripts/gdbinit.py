...
panes = {
    ...
    "stack": os.popen(
        'tmux split-window -v -P -F "#{pane_id}:#{pane_tty}" -l 40% -t {top} -d "cat -"'
    )
    .read()
    .strip()
    .split(":"),
    ...
    "backtrace": os.popen(
        'tmux split-window -h -P -F "#{pane_id}:#{pane_tty}" -t -1 -l 30% -d "cat -"'
    )
    .read()
    .strip()
    .split(":"),
    ...
    "regs": os.popen(
        'tmux split-window -h -P -F "#{pane_id}:#{pane_tty}" -t {top} -l 30% -d "cat -"'
    )
    .read()
    .strip()
    .split(":"),
    "ipython": os.popen(
        'tmux split-window -h -P -F "#{pane_id}:#{pane_tty}" -t {bottom} -l 30% -d "ipython"'
    )
    .read()
    .strip()
    .split(":"),
