...
selected=$(fzf --ansi --multi \
        ...
        --bind "ctrl-f:execute(CWD=\$(cat /tmp/rg-fzf-cwd); awk -F: -v cwd=\"\$CWD\" 'NR==1{printf \"nvim -O +%s %s/%s\", \$2, cwd, \$1} NR>1{printf \" %s/%s\", cwd, \$1}' {+f} > /tmp/rg-nvim-cmd.sh && bash /tmp/rg-nvim-cmd.sh)")
