#!/bin/bash
# sg_fzf_search.sh — ast-grep search with fzf preview + nvim open
#
...
#
# Usage:
#   sg_fzf_search.sh -p '<pattern>' -l <lang> [paths...]   # inline pattern
#   sg_fzf_search.sh -r <rule.yml> [paths...]               # rule file
#   sg_fzf_search.sh -k <kind> -l <lang>                    # kind-based
#
# Examples:
#   sg_fzf_search.sh -p 'def $NAME($$): $$$BODY' -l python
#   sg_fzf_search.sh -p 'console.log($$$ARGS)' -l typescript
#   sg_fzf_search.sh -r ~/script.yml src/
#
...
SELECTED=$(echo "$DATA" \
    | fzf --ansi --multi \
        ...
        --bind "ctrl-f:execute(sg_fzf_search.sh --peek {+1} -- {+2})" \
