...
description: Create Excalidraw diagram JSON files. Use when the user wants to visualize workflows, architectures, concepts, or generate architecture diagrams from a codebase. Covers both abstract diagrams and system architecture from code analysis.
---

# Excalidraw Diagram Creator

Generate `.excalidraw` JSON files that **argue visually**, not just display information. Works for both conceptual diagrams and architecture diagrams derived from codebase analysis.
...
## Codebase Analysis (For Architecture Diagrams)

When the user asks to diagram an existing codebase, discover components before drawing:
...
**Color palettes for infrastructure diagrams** — platform-specific palettes (AWS, Azure, GCP, Kubernetes, Default) are in `references/color-palette.md`.

**Output location:** `docs/architecture/` or user-specified path.
...
| File | Contents |
|------|----------|
| `references/color-palette.md` | Semantic colors, text hierarchy, evidence artifact colors, platform palettes (AWS/Azure/GCP/K8s) |
| `references/element-templates.md` | Copy-paste JSON templates for text, line, dot, rectangle, arrow |
| `references/json-schema.md` | Element types, common properties, text/arrow-specific properties |
| `references/arrows.md` | Edge calculations, routing algorithm, patterns, staggering, bindings |
| `references/validation.md` | Pre-flight algorithm, checklists, common bugs and fixes |
