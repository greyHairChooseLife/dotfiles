...
Pre-flight checklists, validation algorithms, and bug fixes.
...
- [ ] Assessed depth needed (simple/conceptual vs comprehensive/technical)
- [ ] For technical diagrams: looked up actual specs, event names, API formats
- [ ] Identified all components/concepts
- [ ] Mapped all connections and data flows
- [ ] Chose layout pattern (vertical, horizontal, hub-and-spoke, fan-out, etc.)
- [ ] Selected color palette (semantic colors from color-palette.md, or platform-specific)
- [ ] Planned grid positions and section boundaries
- [ ] For large diagrams: planned section-by-section order
...
- [ ] Arrows have `startBinding` and `endBinding` where possible
- [ ] No diamond shapes used (use styled rectangles instead)
- [ ] Applied staggering formula for multiple arrows from same edge
- [ ] Descriptive string IDs used (not numeric seeds)
- [ ] <30% of text elements inside containers (rest are free-floating)

## Checklist: Arrow Validation (Every Arrow)
...
### Bug: Text clipped by container

**Cause**: Container too small for text content.

**Fix**: Increase container width/height, or reduce font size.

### Bug: Elements overlap

**Cause**: Coordinates not spaced sufficiently.

**Fix**: Use layout grid (see Layout Patterns in SKILL.md). Standard spacing: 130-150px between rows, 200-250px between columns.
